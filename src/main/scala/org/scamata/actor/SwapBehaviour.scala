...
/**
  ...
  */
class SwapBehaviour(worker: Worker, rule: SocialRule) extends Agent(worker: Worker, rule: SocialRule) with FSM[State, StateOfMind] with Stash{
  ...
  when(Pause) {
    ...
    case Event(Propose(task, opponentWorkload), mind) =>
      ...
      var workload = worker.workload(mind.bundle, cost)
      val flowtime = directory.peers(worker).foldLeft(0.0)((acc, w) => acc + mind.belief(w))
      if (debug) println(s"$worker receives a proposal $task from $opponent in state pause")
      var updatedBelief = mind.belief + (opponent -> opponentWorkload)
      var found = false
      //var bestSwap : SingleSwap = new SingleSwap(worker, opponent, task, task)
      var bestCounterpart = task
      var bestSwapGoal = rule match {
        case Cmax => Math.max( workload + cost(worker, task), opponentWorkload - cost(opponent, task))
        case Flowtime => flowtime + cost(worker, task) - cost(opponent, task)
      }
      mind.bundle.foreach{ counterpart =>
        //val swap = new SingleSwap(opponent, worker, task, counterpart)
        //val swapBundle = bundle + task - counterpart
        val swapWorkload = workload + cost(worker, task) - cost(worker, counterpart)
        val swapOpponentWorkload = mind.belief(opponent)+cost(opponent, counterpart)
        val swapGoal = rule match {
          case Cmax =>
            Math.max( swapWorkload, swapOpponentWorkload )
          case Flowtime =>
            flowtime + cost(worker, task) - cost(opponent, task) + cost(opponent, counterpart) - cost(worker, counterpart)
        }
        if (swapGoal < bestSwapGoal){
          found = true
          bestCounterpart = counterpart
        }
      }
      if (found) { // Either there is a better swap
        if (debug) println(s"$worker stays in Pause")
        sender ! CounterPropose(task, bestCounterpart, workload)
        goto(Proposer) using new StateOfMind(mind.bundle, updatedBelief)
      }else {// Otherwise
        if (acceptable(task, opponent, worker, updatedBelief)) {// Either the swap is acceptable
          val updatedBundle = mind.bundle + task
          workload = mind.belief(worker) + cost(worker, task)
          val opponentNewWorkload = opponentWorkload - cost(opponent, task)
          updatedBelief = mind.belief + (worker -> workload, opponent -> opponentNewWorkload)
          sender ! Accept(task, workload)
          self ! Give(updatedBundle)
          goto(Pause) using new StateOfMind(updatedBundle, updatedBelief)
        } else { // Or the swap is not acceptable
  ...
  /**
    ...
    */
  when(Proposer) {
    //case Event(Counterproposal(task, counterpart, opponentWorkload), mind) =>
    ...
    //case Event(Accept(task, counterpart, opponentWorkload), mind) =>
    //case Event(Reject(task, counterpart, opponentWorkload), mind) =>
